...
export const POST = adminRoute('catalog:write', async ({ request, user, supabase }) => {
  ...
  const row = unwrap(await supabase.from('products').insert({
    ...
    fabric: input.fabric, care: input.care, category_id: input.categoryId, base_price: input.basePrice, status: input.status,
    featured: input.featured, original_colour_id: input.originalColourId,
  }).select('id').single());
